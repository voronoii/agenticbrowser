"""Agentic Browser — FastAPI + WebSocket 서버

deepagents 기반 에이전트 루프를 실행한다.

실행:
    python -m src.server
    또는
    HEADLESS=true uvicorn src.server:app --host 0.0.0.0 --port 1234
"""
...
from src.agent import create_browser_agent, run_browser_agent
from src.config import (
    SERVER_HOST, SERVER_PORT, HEADLESS, DEFAULT_MODEL, LOG_DIR,
)

# 로그 설정 (reload 시 핸들러 중복 방지)
...
async def run_agent_task(
    ...
    try:
        ...
        shared.ask_human_callback = on_ask_human
